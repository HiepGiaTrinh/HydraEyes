...
from segmentation.segmenation_inference import (
    get_start_end_line,
    segment_gauge_needle,
    get_fitted_line,
    cut_off_line,
)
...
def crop_image(img, box, flag=False, two_dimensional=False):
    ...
    img = np.copy(img)
    if two_dimensional:
        cropped_img = img[box[1] : box[3], box[0] : box[2]]  # image has format [y, x]
    else:
        cropped_img = img[
            box[1] : box[3], box[0] : box[2], :
        ]  # image has format [y, x, rgb]
...
def rescale_ellipse_resize(ellipse_params, original_resolution, resized_resolution):
    ...
    # move ellipse center
    x0_new, y0_new = move_point_resize(
        (x0, y0), original_resolution, resized_resolution
    )
...
def process_image(
    ...
    cropped_resized_img = cv2.resize(
        cropped_img, dsize=RESOLUTION, interpolation=cv2.INTER_CUBIC
    )
    ...
    if debug:
        plotter.plot_zero_point_ellipse(
            np.array(zero_point), np.vstack((start_point, end_point)), ellipse_params
        )
    ...
    if RANDOM_ROTATIONS:
        ...
        res_ellipse_params = rescale_ellipse_resize(
            ellipse_params, RESOLUTION, cropped_img_resolution
        )
    ...
    if debug:
        plotter.plot_ellipse(
            point_needle_ellipse.reshape(1, 2), ellipse_params, "needle_point"
        )
    ...
    result.append({constants.READING_KEY: reading, constants.MEASURE_UNIT_KEY: unit})

    if debug:
        if RANSAC:
            plotter.plot_linear_fit_ransac(
                angle_number_arr,
            ...
            plotter.plot_linear_fit(
                angle_number_arr, (needle_angle_conv, reading), reading_line
            )

        print(f"Final reading is: {reading} {unit}")
        plotter.plot_final_reading_ellipse(
            [], point_needle_ellipse, round(reading, 1), ellipse_params
        )
...
def write_files(result, result_full, errors, run_path, eval_mode):
    ...
    if eval_mode:
        result_full_path = os.path.join(run_path, constants.RESULT_FULL_FILE_NAME)
...
def read_args():
    ...
    parser.add_argument(
        "--base_path", type=str, required=True, help="Path where run folder is stored"
    )
